...
public class UserOnlineConsumer implements RocketMQListener<UserOnlineEvent> {
    ...
    public void onMessage(UserOnlineEvent userOnlineEvent) {
        ...
        logger.info("用户上线:{} 最近一次登录时间:{}",userOnlineEvent.getUserId(),userProfileEvent.getLastLoginTime());
        this.container.online(userOnlineEvent.getUserId(), userProfileEvent.getGmtModified());
        UserModifyParam userModifyParam = new UserModifyParam(userOnlineEvent.getUserId(), userProfileEvent.getGmtModified(), userProfileEvent.getIp(), userProfileEvent.getStatus());
        try {
            logger.info("更新用户上线状态");
